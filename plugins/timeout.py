...
class TimeoutCommand(commands.Cog):
    ...
    @commands.slash_command(description="Times out a specified member with an optional reason")
    async def timeout(self, ctx, member: disnake.Member, *, reason="Unspecified reason", days=0, hours=1, minutes=0, seconds=0):
        if member.id == ctx.author.id:
            ...
            reasonEmbed = disnake.Embed(
                ...
                description=f'Succesfully timed out {member.mention} for {reason} for {days} days, {hours} hours, {minutes} minutes, {seconds} seconds\n \n ',
                colour=0xFF0000
            )
            ...
            reasonEmbed.set_footer(
                text=f"Timed out by {ctx.author.name}", icon_url='{}'.format(ctx.author.avatar))
